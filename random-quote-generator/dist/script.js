        ...
        const cioranWisdom = {  
          ...
          quotePartTwo: ['the fact that life has no meaning is a reason to live - moreover, the only one.', 'that a human being is a robot with defects', 'if we had the courage to confront the doubts we timidly conceive about ourselves, none of us would utter an "I" without shame', 'knowledge subverts love: in quoteortion as we penetrate our secrets, we come to loathe our kind, precisely because they resemble us', 'we are so lonely in life that we must ask ourselves if the loneliness of dying is not a symbol of our human existence','my mission is to kill time, and time is to kill me in its turn. How comfortable one is among murderers', 'that every man sleeps a prophet, and when he wakes there is a little more evil in the world'],
...
// Store Cioran's quote in an array
let cioranQuote = []
...
for(let quote in cioranWisdom) {
  ...
  switch(quote) {
    case 'quotePartOne':
      cioranQuote.push(`🕯️ "${cioranWisdom[quote][index]}.`)
      break
    case 'quotePartTwo':
